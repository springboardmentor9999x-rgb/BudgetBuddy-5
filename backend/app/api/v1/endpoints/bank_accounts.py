...
def create_bank_account(
    ...
    # Check duplicate bank account number guard
    existing_bank = db.query(BankAccount).filter(
        BankAccount.user_id == current_user.id,
        BankAccount.bank_name == bank_in.bank_name,
        BankAccount.account_number_last4 == bank_in.account_number_last4
    ).first()
    if existing_bank:
        raise HTTPException(
            status_code=400,
            detail=f"A bank account ending in '{bank_in.account_number_last4}' for '{bank_in.bank_name}' is already linked to your profile!"
        )
    ...
    new_bank = BankAccount(
        ...
        account_limit=bank_in.account_limit or 0.0,
